...
def _binary_dice_hd(pred_b: np.ndarray, gt_b: np.ndarray):
    """Return Dice and full Hausdorff distance for binary masks."""
    ...
    if pred_b.sum() > 0 and gt_b.sum() > 0:
        ...
        return float(dice), float(hd_full)
...
def validate(
    ...
    with torch.no_grad():
        for _, data in enumerate(val_dataloader):
            ...
            for b in range(sp.shape[0]):
                pred_b = sp[b] > 0
                gt_b = gt[b] > 0
                dice_i, hd_i = _binary_dice_hd(pred_b, gt_b)
    ...
    fabric.print(
        ...
        f"SemF1:[{sem_f1_scores.avg:.4f}] SemHD:[{sem_hds.avg:.4f}] "
