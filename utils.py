...
def calc_metric(pred: np.ndarray, gt: np.ndarray):
    """
    Compute per-image Dice / HD / HD95 for fragment semantic segmentation.
...
# ===================== Fragment semantic evaluation: Dice / HD / HD95 =====================
...
def evalSemantic(coco, res_json: str, return_hd: bool = False):
    ...
    for imgId in imgIds:
        ...
        dice_i, hd_i, hd95_i = calc_metric(pred, label)
        dice += dice_i
        hd += hd_i
        hd95 += hd95_i
        num += 1
    ...
    mean_hd95 = hd95 / max(num, 1)
    print("dice:", mean_dice, "\n", "hd:", mean_hd, "\n", "hd95:", mean_hd95)
    if return_hd:
        return mean_dice, mean_hd, mean_hd95
